...
% 2 parent genotypes - no parents
for k = 1:numPeople
  for j = 1:2
    k_p = pedigree.parents(k,j);
    if k_p == 0
      index = k + numPeople * (j - 1);
      factorList(index) = childCopyGivenFreqsFactor(alleleFreqs, index);
    end
  end
end

% 2 parent genotypes - from parents
for k = 1:numPeople
  
  k_p1 = pedigree.parents(k,1);
  if k_p1 ~= 0 
    factorList(k) = childCopyGivenParentalsFactor( ...
      numAlleles, k, k_p1, k_p1 + numPeople);
  end
  
  k_p2 = pedigree.parents(k,2);
  if k_p2 ~= 0 
    j = k + numPeople;
    factorList(j) = childCopyGivenParentalsFactor( ...
      numAlleles, j, k_p2, k_p2 + numPeople);
  end
...
for k1 = 1:numPeople
  k2 = k1 + numPeople;
  k3 = k1 + 2*numPeople;
  factorList(k3) = phenotypeGivenCopiesFactor( ...
    alphaList, numAlleles, k1, k2, k3);
end
