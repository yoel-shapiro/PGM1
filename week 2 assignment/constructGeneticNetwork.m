...
% genotypes - no parents
for k = 1:numPeople
  if all(pedigree.parents(k,:) == [0 0])
    factorList(k) = genotypeGivenAlleleFreqsFactor(alleleFreqs, k);
  end
end

% genotypes - from parents
for k = 1:numPeople
  if any(pedigree.parents(k,:) ~= 0)
    factorList(k) = genotypeGivenParentsGenotypesFactor( ...
      numAlleles, k, pedigree.parents(k,1), pedigree.parents(k,2));    
  end
end

% phenotypes
for k = (numPeople+1):(2*numPeople)
  factorList(k) = phenotypeGivenGenotypeFactor(alphaList, (k-numPeople), k);
end
